...
# OpenAI API Key
openai.api_key = os.environ.get("OPENAI_API_KEY", "your-openai-api-key") # Use environment variable
...
class RecentActivity(BaseModel):
    user: str
    action: str
    timestamp: datetime # Changed to datetime to match DB model
...
async def register_user(user: UserInDB, db: Session = Depends(get_db)):
    ...
    db_user = DBUser(username=user.username, email=user.email, full_name=user.full_name, hashed_password=hashed_password, disabled=user.disabled)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
async def get_dashboard_data(db: Session = Depends(get_db)): # current_user: User = Depends(get_current_active_user)
    metrics = db.query(DBDashboardMetric).all()
    recent_activities = db.query(DBRecentActivity).order_by(DBRecentActivity.timestamp.desc()).limit(3).all()
    performance_chart = db.query(DBChartData).first()

    return DashboardData(
        metrics=metrics,
        recent_activities=recent_activities,
        performance_chart=performance_chart if performance_chart else ChartData(labels=[], values=[])
    )
...
if __name__ == "__main__":
    # This part is for local development and initial table creation
    # In production, Render will handle running the app and migrations
